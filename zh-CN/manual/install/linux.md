...
本安装教程编写时的系统环境为 AMD64 上的 Ubuntu 22.04、Debian 12。
...
:::tabs code
```dpkg
apt update && apt install -y git adb netcat libgomp1
```
```rpm
yum update && yum install -y git adb netcat libgomp
```
:::
...
:::tip
如果实在无法安装 `Python 3.7.6`，可以安装`Python 3.8.10`，或者按照[从源码安装 python](#从源码安装-python)来安装
:::
...
编译 Python 需要 `gcc`、`zlib`、`libffi-devel`和`make`。以及我们使用编译后的 pip 去下载依赖的时候，使用公网镜像源如清华源的时候，必须要使用 SSL，因此还需要编译后的 `openssl` 库文件。我们先安装工具
:::tabs code
```dpkg
apt update && apt install -y gcc zlib1g-dev make libffi-dev
```
```rpm
yum update && yum install -y gcc zlib make libffi libffi-devel
```
:::
### 获取 openssl 源码

在 openssh [下载页面](https://openssl.org/source/)找到并下载源码。
```sh
wget https://openssl.org/source/openssl-3.1.1.tar.gz
```
下载完成后解压。
...
进入 openssl 文件夹，初始化编译参数。噢是的，这是个可执行文件，执行它！
...
creating Makefile
```
可以看到第 5 行以及第 7 行结果都为yes，表示初始化脚本成功找到了需要链接的 openssl 库文件。请确保 1-6 行至少有一个为 yes 的结果。当然，检测到库文件之后就不会继续检测下去了，所以第 6 行这种正常是不会显示出来的。如果没有这个结果就进行下面的步骤只会被编译器狠狠地骂
...
为了不影响原有的 python3 环境，这里我们删除 `/usr/local/bin` 下的 `python3` 软链接。
...
查看版本。
```sh
python38 --version && pip38 --version
```
```sh
Python 3.8.10
